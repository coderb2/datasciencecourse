...
unzip(zipfile="dataDataset.zip")

#define the path where the new folder has been unziped
pathdata="c:/Anushree/Coursera/UCI HAR Dataset"
#create a file which has the 28 file names
files = list.files("c:/Anushree/Coursera/UCI HAR Dataset",recursive=TRUE)xtest = read.table(file.path(pathdata, "test", "X_test.txt"),header = FALSE)
